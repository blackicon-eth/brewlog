...
export type CoffeeWithStats = Coffee & { brewCount: number; avg: number | null; lastBrewedAt: number | null };
...
// brew-less coffee. Newest-first, mirroring listCoffees.
export async function listCoffeesWithStats(db: Db): Promise<CoffeeWithStats[]> {
  const rows = await db.getAllAsync<
    CoffeeRow & { brew_count: number; avg_rating: number | null; last_brewed_at: number | null }
  >(
